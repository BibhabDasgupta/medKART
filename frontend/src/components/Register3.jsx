...
const Register3 = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:5000/api/distributors/register`, formData);
      if (response.data) {
        navigate(`/distributors/login`); // Redirect to login after successful registration
      }
    } catch (error) {
  ...
  return (
    <div className="register-card">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <p>
          Already have an account? <Link to="/distributors/login">Login here</Link>
        </p>
      </form>
    </div>
  );
