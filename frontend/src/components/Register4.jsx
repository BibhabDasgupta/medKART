...
const Register4 = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:5000/api/hospitalspharmacies/register`, formData);
      if (response.data) {
        navigate(`/hospitalspharmacies/login`); // Redirect to login after successful registration
      }
    } catch (error) {
  ...
  return (
    <div className="register-card">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <p>
          Already have an account? <Link to="/hospitalspharmacies/login">Login here</Link>
        </p>
      </form>
    </div>
  );
