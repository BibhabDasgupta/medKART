    ...
    try {
      ...
      if (networkData) {
        ...
        if (currentAccount !== owner) {
          console.error('You are not authorized to register Wholesaler.');
    ...
    try {
      ...
      await contract.methods.registerWholesaler(username, accountNumber).send({ from: account })
        ...
        .on('receipt', (receipt) => {
          ...
          this.setState({ successMessage: `Wholesaler ${username} registered with account number ${accountNumber}` });
